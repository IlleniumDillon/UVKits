...
double RRT_STAR::res_distance(double p,double direction)
{
    if(direction>0)
    {
        return (1-fmod(p,1));
...
void RRT_STAR::SetMap(double _resolution, int max_x_id, int max_y_id, int max_z_id,Vector3d _origin)
{
...
void RRT_STAR::ResetRRT(Vector3d start, Vector3d goal)
{
    node_list.clear();
    start_data = start;
    goal_data = goal;
...
RRT_STAR_Node* RRT_STAR::Sample()
{
    RRT_STAR_Node* x_rand = nullptr;
    if (goal_dis(goal_gen) > goal_sample_rate) { //如果生成的随机数大于目标采样概率，则生成随机点
        ...
        x_rand = new RRT_STAR_Node(Vector3d(randX,randY,randZ));//存储生成的随机位置

    }
    else 
    {   //否则目标点则为随机点
        x_rand = new RRT_STAR_Node(goal_data);
    }
    return x_rand;
}

RRT_STAR_Node* RRT_STAR::Near(RRT_STAR_Node* x_rand)
{
    int min_index = -1;
...
void RRT_STAR::fixInMap(Eigen::Vector3d& new_node)
{
...
RRT_STAR_Node* RRT_STAR::Step(RRT_STAR_Node* x_rand, RRT_STAR_Node* x_near)
{
    ...
    fixInMap(vec_new);
    RRT_STAR_Node* x_new = new RRT_STAR_Node(vec_new); 
    return x_new;
}

void RRT_STAR::AddNode(RRT_STAR_Node* x_new, RRT_STAR_Node* x_father)
{
    x_new->fatherPtr = x_father;
    node_list.push_back(x_new);
}

bool RRT_STAR::SuccessCheck(RRT_STAR_Node* x_new)
{
...
bool RRT_STAR::CollisionFree( RRT_STAR_Node* x_near,RRT_STAR_Node* x_new)
{
    //得到栅格坐标,非负
...
std::vector<Vector3d> RRT_STAR::Planning()
{
    RRT_STAR_Node* x_rand  = nullptr;
    RRT_STAR_Node* x_near  = nullptr;
    RRT_STAR_Node* x_new  = nullptr;
...
Vector3d RRT_STAR::map2rviz(const Vector3d& index) const
{
    Vector3d pt;
    pt = index + origin;
    return pt;
...
Vector3d RRT_STAR::rviz2map(const Vector3d& pt) const
{
    Vector3d pt_;
